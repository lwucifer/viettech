<!-- ref:https://github.com/MPComponent/mpvue-picker/blob/master/src/mpvue-picker/mpvuePicker.vue -->
<template>
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content" :class="{'mpvue-picker-view-show':showPicker}">
            <view class="mpvue-picker__hd" catchtouchmove="true">
                <view class="mpvue-picker__action" @click="pickerCancel">{{$t('common.cancel')}}</view>
                <view class="mpvue-picker__action" :style="{color:themeColor}" @click="pickerConfirm">{{$t('common.confirm')}}</view>
            </view>
            <!-- 单列 -->
            <picker-view indicator-style="height: 40px;"
...
<script>
    import { transformDateToIndex, getDatePickerIndex, getDays, fixPickerValueDefault } from './m-picker-data.js';
        ...
        methods: {
                    ...
                    if (this.pickerValueArrayChange || this.modeChange) {
                        ...
                        if (val) {
                            ...
                            if (this.mode === 'dateSelector') {
                                this.pickerValue = getDatePickerIndex(val)
                            }
                        }
                    }
                }, 0);
            ...
            _getPickerLabelAndValue(value, mode) {
                ...
                if (mode === 'selector') {
                    ...
                    pickerLable =
                        `${this.pickerValueYear[value[0]].label}-${PrefixZero(this.pickerValueMonth[value[1]].label, 2)}-${PrefixZero(this.pickerValueDay[value[2]].label,2)}`;
...
<style>
    .pickerMask { position: fixed; z-index: 1000; top: 0; right: 0; left: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); }
    .mpvue-picker-content { position: fixed; bottom: 0; left: 0; width: 100%; transition: all 0.3s ease; transform: translateY(100%); z-index: 3000; opacity: 0; }
    .mpvue-picker-view-show { transform: translateY(0); opacity: 1; }
    .mpvue-picker__hd { display: flex; padding: 9px 15px; background-color: #fff; position: relative; text-align: center; font-size: 17px; }
        .mpvue-picker__hd:after { content: ' '; position: absolute; left: 0; bottom: 0; right: 0; height: 1px; border-bottom: 1px solid #e5e5e5; color: #e5e5e5; transform-origin: 0 100%; transform: scaleY(0.5); }
    .mpvue-picker__action { display: block; flex: 1; color: #1aad19; }
        .mpvue-picker__action:first-child { text-align: left; color: #888; }
        .mpvue-picker__action:last-child { text-align: right; }
    .picker-item { text-align: center; line-height: 40px; font-size: 16px; }
    .mpvue-picker-view { position: relative; bottom: 0; left: 0; width: 100%; height: 238px; background-color: rgba(255, 255, 255, 1); }
</style>